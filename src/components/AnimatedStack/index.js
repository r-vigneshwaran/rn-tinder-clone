...
const AnimatedStack = ({data, renderItems, onSwipeLeft, onSwipeRight}) => {
  ...
  const LikeStyle = useAnimatedStyle(() => ({
    opacity: interpolate(translateX.value, [0, outOfScreen / 10], [0, 1]),
  }));
  const NopeStyle = useAnimatedStyle(() => ({
    opacity: interpolate(translateX.value, [-outOfScreen / 10, 0], [1, 0]),
  }));
